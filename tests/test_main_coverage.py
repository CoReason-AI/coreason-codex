...
def test_cli_serve_success(tmp_path: Path) -> None:
    pack_dir = tmp_path / "codex_pack"
    pack_dir.mkdir()

    # Patch where uvicorn is imported in main.py
    with patch("coreason_codex.main.uvicorn.run") as mock_run:
        mock_env: Dict[str, str] = {}
        with patch("coreason_codex.main.os.environ", mock_env):
...
def test_cli_serve_failure(tmp_path: Path) -> None:
    pack_dir = tmp_path / "codex_pack"
    pack_dir.mkdir()
