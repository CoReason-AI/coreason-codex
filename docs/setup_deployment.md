...
### Step 1.2: CPT4 Hydration (Important)
If you selected **CPT4** (Current Procedural Terminology), you **must** restore the concept names using the utility provided in the download.

1.  Athena provides a `cpt4.jar` (or similar script) in the zip file.
2.  You must run this utility to populate the CPT4 concepts in `CONCEPT.csv`.
3.  Command example:
    ```bash
    java -Dopen_source_folder=./ -jar cpt4.jar
    ```
    *(See the `readme.txt` included in your Athena download for exact instructions.)*

### Step 1.3: Verify Required Files
When you unzip the download (and finish CPT4 hydration), ensure you have the following **exact CSV files**:
